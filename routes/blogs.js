...
module.exports = (router)=>{
    ...
    router.get('/allBlogs', (req,res)=>{
        ...
        }).sort({'_id': -1}); //sort by id, -1 means descending order
    })
                                   ...
                                   if(user.username !== blog.createdBy){
                                       ...
                                       blog.remove((err)=>{
                                           if(err){
                                               ...
                                               res.json({success: true, message: 'Blog delete!'})
                                           }
                                       })
                                   }
                               }
                            ...
                            User.findOne({_id : req.decoded.userId}, (err, user)=>{
                                if(err){
                                    ...
                                    if(!user){
                                        ...
                                        blog.save((err)=>{
                                            if(err){
                                                req.json({success: false, message: 'Something went wrong with saving your comment'})
                                            }else{
